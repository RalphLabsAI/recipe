...
class RalphConfig:
    ...
    dropout: float = 0.0          # residual dropout (NO params; training-only regularizer)
    logit_z_coef: float = 0.0     # #1317 z-loss coefficient (NO params; loss-only regularizer)
...
class Block(nn.Module):
    def __init__(self, cfg: RalphConfig):
        ...
        self.ffn = SwiGLU(cfg)
        self.drop = nn.Dropout(getattr(cfg, "dropout", 0.0))  # param-free residual dropout
...
class RalphBase(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if targets is not None:
            ...
            zc = getattr(self.cfg, "logit_z_coef", 0.0)  # #1317 z-loss: pull logsumexp toward 0
            if zc:
