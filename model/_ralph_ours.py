...
class Attention(nn.Module):
    def __init__(self, cfg: RalphConfig):
        ...
        # Lever 4: learned per-head softmax temperature (1D => AdamW no-decay).
        # exp(0)=1 => identity at init; lets each head sharpen/soften attention
        # without fighting Muon's spectral clamp on qkv.
        self.attn_temp = nn.Parameter(torch.zeros(cfg.n_heads))
    ...
    def forward(self, x: torch.Tensor, rope_cache: torch.Tensor) -> torch.Tensor:
        ...
        q = q * torch.exp(self.attn_temp).view(1, -1, 1, 1)  # Lever 4: per-head temp
...
class SwiGLU(nn.Module):
    def __init__(self, cfg: RalphConfig):
        ...
        # Lever 2: RMSNorm the gated hidden before w_down (sub-LN). Restores
        # per-token conditioning so more MLP channels carry signal.
        self.hidden_norm = RMSNorm(hidden, cfg.rms_norm_eps)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.w_down(self.hidden_norm(F.silu(self.w_gate(x)) * self.w_up(x)))
