...
class RalphConfig:
    ...
    value_residual: bool = True   # ResFormer: learnable per-layer shortcut to layer-0 attn values
...
class Attention(nn.Module):
    ...
    def forward(self, x, rope_cache, v0=None, value_lambda=None):
        B, T, C = x.shape
        ...
        v_self = v  # this layer's own value (layer-0's is cached as the residual source)
        if v0 is not None and value_lambda is not None:
            v = v + value_lambda * v0  # value-residual: additive shortcut to layer-0 values
...
class Block(nn.Module):
    ...
    def forward(self, x, rope_cache, v0=None, value_lambda=None):
        attn_out, v_self = self.attn(self.attn_norm(x), rope_cache, v0, value_lambda)
        x = x + attn_out
...
class RalphBase(nn.Module):
    ...
    def __init__(self, cfg: RalphConfig):
        ...
        # value-residual: one 0-init learnable scalar per layer (index 0 unused).
        self.value_residual = getattr(cfg, "value_residual", False)
        if self.value_residual:
            self.value_lambda = nn.Parameter(torch.zeros(cfg.n_layers))
    ...
    def _forward_impl(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        v0 = None
        vr = self.value_residual
        if self.unet_skip:
            n = len(self.blocks); half = n // 2; enc = []
            for i, block in enumerate(self.blocks):
                lam = self.value_lambda[i] if (vr and i > 0) else None
                if i < half:
                    x, v_self = block(x, self.rope_cache, v0, lam); enc.append(x)
                else:
                    x = x + self.skip_gate[i - half] * enc[n - 1 - i]
                    x, v_self = block(x, self.rope_cache, v0, lam)
                if i == 0:
                    v0 = v_self
        else:
            for i, block in enumerate(self.blocks):
