...
class RalphConfig:
    ...
    logit_z_coef: float = 0.0     # z-loss: coef * mean(logsumexp(logits)^2); 0 = off
...
class RalphBase(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if targets is not None:
            ...
            if z_coef:
                # z-loss (PaLM): keeps log Z near 0, regularizing the readout
                # calibration; fp32 for the logsumexp regardless of logit dtype.
                loss = loss + z_coef * (torch.logsumexp(logits.float(), dim=-1) ** 2).mean()
        return logits, loss
