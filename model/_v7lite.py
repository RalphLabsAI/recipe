...
class RalphConfig:
    ...
    # mixing scalars. Param-heavy but compute-cheap (an embedding gather, no
    # matmul); a scale-robust val_bpb win that stacks on the v4/v7 stack.
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope_cache: torch.Tensor,
                ...
                ve_lam: Optional[torch.Tensor] = None) -> torch.Tensor:
        B, T, C = x.shape
        ...
        # recipe-v5: value embeddings — blend the per-token value-table lookup
        # into V with the layer's learnable mixing scalars (v is not rotated,
        # so blend here).
        if ve is not None:
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope_cache: torch.Tensor,
                ve: Optional[torch.Tensor] = None,
                ve_lam: Optional[torch.Tensor] = None) -> torch.Tensor:
        x = x + self.attn(self.attn_norm(x), rope_cache, ve, ve_lam)
...
class RalphBase(nn.Module):
    ...
    def __init__(self, cfg: RalphConfig):
        ...
        # Learned scalar readout temperature. The embedding/head norms set the
        # logit scale implicitly; this single scalar decouples the readout gain
        # so the softmax temperature is fit directly. exp() parameterisation
        # keeps it strictly positive; 0-init => gain 1.0 (identity at step 0).
        # Shape () => routed to AdamW(no-decay) by build_optimizer.
        self.logit_scale = nn.Parameter(torch.zeros(()))
        # Per-vocab readout calibration (readcal): per-token multiplicative gain
        # + additive bias on the head. 0-init => exp(0)=1 and +0 => identity at
        # step 0. Both shape (vocab,) => 1D => AdamW(no-decay).
    ...
    def _forward_impl(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        # recipe-v5 readout calibration: per-vocab gain (per-token temperature)
        # + bias (per-token prior), then the global temperature, all before the
        # soft-cap. exp(0)=1 and +0 at init => identity at step 0.
