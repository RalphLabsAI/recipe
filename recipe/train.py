...
class TrainConfig:
    ...
    dropout: float = 0.0          # param-free residual dropout (regularizer for >1 epoch)
    logit_z_coef: float = 0.0     # param-free z-loss coefficient
    ...
    # Weight EMA (Polyak averaging) of the SAVED checkpoint. When ema_decay is set,
    # maintain an fp32 shadow of the params updated each step once
    # step/total_steps >= ema_start_frac, and SAVE the shadow (not the raw final
    # iterate) as checkpoint.pt. ema_start_frac defaults to the WSD decay onset
    # (1 - decay_frac) so only the low-LR tail is averaged. No effect when
    # ema_decay is None/0 (legacy behaviour). Lowers held-out val_bpb by landing
    # on a flatter, lower-variance point of the same trajectory.
    ...
    fast_kernels: bool = False  # non-deterministic cuDNN autotune + TF32, set in train() before compile
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    if getattr(cfg, "fast_kernels", False):
        # Declared throughput path: pin non-deterministic cuDNN autotune + TF32
        # ONCE here, BEFORE any torch.compile capture, so use_deterministic_algorithms
        # never flips mid-forward (which torch>=2.12 rejects as a fwd/bwd mismatch).
        # GPU training is already non-bit-exact and the validator audit is tolerance-based.
    ...
    # Weight-EMA (Polyak) shadow, saved in place of the raw final iterate. Float
    # params tracked in fp32; non-float buffers kept in native dtype (no round-trip).
    ...
    for step in range(cfg.total_steps):
        ...
        # Weight-EMA: track the live weights until decay onset, then exponentially
        # average the low-LR tail. Runs under no_grad inside the timed loop; the
        # logged loss/tokens/elapsed below are from the LIVE model (EMA never
        # touches the trajectory), so op3/integrity identities are unchanged.
        if ema is not None:
            with torch.no_grad():
                for k, v in model.state_dict().items():
    ...
    # Save the EMA (averaged) weights as the graded checkpoint when EMA is on, cast
    # back to each reference param's dtype so op4's strict load_state_dict into
    # RalphBase is byte-key-identical to a raw save. Legacy raw save when EMA off.
