...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # Bare-cm data loading: rebuild the manifest from data/shards at runtime
    # when the file is absent (cm hashes the source tree only; shards aren't hashed).
    ...
    if not _mp.exists():
        ...
        _man = build_manifest(track="llm-pretraining-launch", tokenizer="gpt2",
            vocab_size=cfg.vocab_size, dtype="uint16", shards=_shards, base_dir=_base)
        _mp = _base / "data_manifest.json"; _man.write(_mp); cfg.manifest_path = str(_mp)
