...
class TrainConfig:
    ...
    # Model hyperparams exposed to config (model already supports these; default = model default)
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # final_state.config must stay container-relative for op1 gate-4: the runner
    # pins --manifest/--data-base-dir to absolute realpaths and train.py records
    # them verbatim. Map each back to its canonical relative tail (from the last
    # "data" path segment). Training already LOADED from the absolute paths above;
    # this only rewrites what gets RECORDED. Independent per-field so it works when
    # only one of the two is pinned absolute (e.g. --manifest without --data-base-dir).
    _cfg_rec = asdict(cfg)
    def _rel_tail(_p):
        try:
            _pp = Path(_p)
            ...
            _parts = _pp.parts
            if "data" in _parts:
                _i = len(_parts) - 1 - _parts[::-1].index("data")
                return Path(*_parts[_i:]).as_posix()
            return _pp.name
        except (ValueError, OSError, TypeError):
            return _p
    _cfg_rec["data_base_dir"] = _rel_tail(cfg.data_base_dir)
    _cfg_rec["manifest_path"] = _rel_tail(cfg.manifest_path)
    summary = {
