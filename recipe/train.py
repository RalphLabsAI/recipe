...
class TrainConfig:
    ...
    unigram_readout_prior: bool = False
    unigram_prior_strength: float = 1.0
    unigram_prior_alpha: float = 1.0
...
def _init_unigram_readout_prior(model: torch.nn.Module, ds: TokenShardDataset, cfg: TrainConfig, device: torch.device) -> None:
    if not getattr(cfg, "unigram_readout_prior", False):
        return
    if not hasattr(model, "readout_bias"):
        return
    import numpy as np

    counts = np.full(cfg.vocab_size, float(getattr(cfg, "unigram_prior_alpha", 1.0)), dtype=np.float64)
    for shard in ds._shards:
        counts += np.bincount(np.asarray(shard, dtype=np.int64), minlength=cfg.vocab_size)
    probs = counts / counts.sum()
    log_prior = np.log(probs)
    log_prior -= log_prior.mean()
    bias = torch.tensor(
        log_prior * float(getattr(cfg, "unigram_prior_strength", 1.0)),
        dtype=model.readout_bias.dtype,
        device=device,
    )
    with torch.no_grad():
        model.readout_bias.copy_(bias)
    print(
        f"[train] unigram_readout_prior strength={cfg.unigram_prior_strength} "
        f"alpha={cfg.unigram_prior_alpha} bias_std={float(bias.float().std().item()):.4f}",
        flush=True,
    )
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    _init_unigram_readout_prior(model, ds, cfg, device)
