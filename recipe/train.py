...
def cosine_lr(step: int, cfg: TrainConfig) -> float:
    ...
    # linear cooldown to min_lr over the final 20% of steps. Replaces cosine so
    # the model spends more steps near the effective peak before annealing.
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # Bare-cm data loading: the canonical container_measurement hashes the
    # source tree only; a data_manifest.json under data/ would pollute it.
    # The shards (.bin) are NOT hashed, so when the manifest file is absent we
    # rebuild it from the shards at runtime (deterministic given the shards).
    _mp = Path(cfg.manifest_path)
    if not _mp.exists():
        from data.manifest import build_manifest
        _base = Path(cfg.data_base_dir)
        _shards = sorted((_base / "shards").glob("*.bin"))
        _man = build_manifest(
            track="llm-pretraining-launch", tokenizer="gpt2",
            vocab_size=cfg.vocab_size, dtype="uint16",
            shards=_shards, base_dir=_base,
        )
        _mp = _base / "data_manifest.json"
        _man.write(_mp)
        cfg.manifest_path = str(_mp)
