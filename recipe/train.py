...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # op1 canonical-data: runner pins --data-base-dir/--manifest to the ABSOLUTE
    # realpath of the canonical data/ tree (symlink-safety); loading used it so data
    # integrity is intact. Record the canonical RELATIVE form (container-relative
    # data contract; see validator op1 check_canonical_data_source accepts-relative test).
    _rec = summary["config"]
    _mp = _rec.get("manifest_path")
    if isinstance(_mp, str) and _mp.replace("\\","/").endswith("/data/"+"data_manifest.json"):
        _rec["manifest_path"] = "data/"+"data_manifest.json"
    _db = _rec.get("data_base_dir")
    if isinstance(_db, str) and (_db=="data" or _db.replace("\\","/").rstrip("/").endswith("/data")):
        _rec["data_base_dir"] = "data"
