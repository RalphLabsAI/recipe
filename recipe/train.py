...
class TrainConfig:
    ...
    data_seed: int = 1337
    init_seed: int = 1337
    data_sampling: str = "replacement"  # "permutation" = without-replacement full-coverage epoch order (seed-keyed); "replacement" = canonical
...
def build_model(cfg: TrainConfig) -> RalphBase:
    return RalphBase(RalphConfig(
        ...
        logit_z_coef=getattr(cfg, "logit_z_coef", 0.0),
    ))
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # Fast kernels want determinism OFF (see model._enable_fast_kernels); force it here
    # so the state is consistent between compiled forward and backward (torch 2.12 check).
    ...
    ds = TokenShardDataset(cfg.manifest_path, cfg.data_base_dir, cfg.seq_len, cfg.data_seed, sampling=getattr(cfg, "data_sampling", "replacement"))
