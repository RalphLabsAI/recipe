...
class TrainConfig:
    ...
    optimizer: str = "muon"
    muon_lr: float = 0.02  # tuned on real FineWeb 125M/500step (0.02 best; −0.020 val_bpb vs 0.04)
    muon_momentum: float = 0.95
    muon_ns_steps: int = 5

    # LR schedule. "wsd" = warmup-stable-decay (hold max_lr, then linearly decay
    # to min_lr over the last `wsd_decay_frac` of training); strongly beats cosine
    # at fixed budget (~−0.11 val_bpb at h100_proxy scale). "cosine" = legacy.
    lr_schedule: str = "wsd"
...
def wsd_lr(step: int, cfg: TrainConfig) -> float:
    """Warmup-stable-decay: linear warmup, hold max_lr, then linear decay to
    ...
    return cfg.max_lr + (cfg.min_lr - cfg.max_lr) * min(1.0, p)


def schedule_lr(step: int, cfg: TrainConfig) -> float:
    return wsd_lr(step, cfg) if cfg.lr_schedule == "wsd" else cosine_lr(step, cfg)
