...
def build_optimizer(model: torch.nn.Module, cfg: TrainConfig):
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        is_embed = ("tok_embed" in n) or ("lm_head" in n) or ("value_embed" in n)
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    log_f.close()
    torch.save({"model": {k: v.to(torch.bfloat16) for k, v in model.state_dict().items()}, "config": asdict(cfg)}, out_dir / "checkpoint.pt")
