...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    if os.environ.get("RALPH_NO_COMPILE", "0") != "1":
        # Inductor needs a C/C++ compiler; point it at the ones on PATH (bare names,
        # resolved via the runtime's PATH — no host path). Without this the proof
        # subprocess can't find a compiler and silently falls back to eager.
