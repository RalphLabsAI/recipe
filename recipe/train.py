...
def build_optimizer(model: torch.nn.Module, cfg: TrainConfig) -> list[torch.optim.Optimizer]:
    ...
    decay_params = [p for n, p in model.named_parameters() if p.requires_grad and p.dim() >= 2 and id(p) not in _eids]
    no_decay_params = [p for n, p in model.named_parameters() if p.requires_grad and p.dim() < 2 and id(p) not in _eids]
    adamw = torch.optim.AdamW(
        [
    ...
    adamw.param_groups[-1]["base_lr"] = cfg.max_lr * 10.0  # embed 10x (m1)
    return [adamw]
