...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    # training LOADED from the runner's absolute pinned paths above (honest
    # canonical data); the RECORDED config keeps the container-relative form so
    # op1's canonical-source check passes, and only when the manifest sits
    # directly inside the pinned base dir (out-of-tree stays absolute).
    _cfg_rec = asdict(cfg)
    try:
