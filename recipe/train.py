...
class TrainConfig:
    ...
    # Logging
    log_every: int = 10
    tail_ema_decay: float = 0.0
    tail_ema_start_frac: float = 0.85
    fixed_unigram_prior: bool = False
    fixed_unigram_prior_tokens: int = 0
    fixed_unigram_prior_scale: float = 1.0
    fixed_unigram_prior_clip: float = 8.0
...
def _init_fixed_unigram_prior(model: torch.nn.Module, cfg: TrainConfig) -> None:
    if not getattr(cfg, "fixed_unigram_prior", False):
        return
    if not hasattr(model, "logit_prior"):
        return
    manifest_path = Path(cfg.manifest_path)
    data_base = Path(cfg.data_base_dir)
    if not manifest_path.is_absolute():
        manifest_path = Path.cwd() / manifest_path
    if not data_base.is_absolute():
        data_base = Path.cwd() / data_base
    meta = json.loads(manifest_path.read_text())
    dtype_name = meta.get("dtype", "uint16")
    dtype = np.uint16 if dtype_name == "uint16" else np.int32
    counts = np.ones(int(cfg.vocab_size), dtype=np.float64)
    limit = int(getattr(cfg, "fixed_unigram_prior_tokens", 0) or 0)
    seen = 0
    for shard in meta.get("shards", []):
        if limit and seen >= limit:
            break
        rel = shard.get("relpath") or shard.get("path")
        if not rel:
            continue
        remaining = -1 if not limit else max(0, limit - seen)
        arr = np.fromfile(data_base / rel, dtype=dtype, count=remaining if remaining else -1)
        if arr.size == 0:
            continue
        arr = arr.astype(np.int64, copy=False)
        counts += np.bincount(arr, minlength=int(cfg.vocab_size))[: int(cfg.vocab_size)]
        seen += int(arr.size)
    log_prior = np.log(counts)
    log_prior = log_prior - log_prior.mean()
    log_prior = np.clip(
        log_prior,
        -float(cfg.fixed_unigram_prior_clip),
        float(cfg.fixed_unigram_prior_clip),
    )
    log_prior = log_prior * float(cfg.fixed_unigram_prior_scale)
    with torch.no_grad():
        model.logit_prior.copy_(torch.tensor(log_prior, device=model.logit_prior.device, dtype=torch.float32))
    print(
        f"[train] fixed_unigram_prior tokens={seen:,} scale={cfg.fixed_unigram_prior_scale} "
        f"std={float(model.logit_prior.float().std().item()):.4f}",
        flush=True,
    )
...
def train(cfg: TrainConfig, out_dir: Path, use_wandb: bool = False) -> dict:
    ...
    _init_fixed_unigram_prior(model, cfg)
    ...
    if getattr(cfg, "tail_ema_decay", 0.0):
        print(f"[train] tail EMA active decay={cfg.tail_ema_decay} start_frac={cfg.tail_ema_start_frac}", flush=True)
    if wb_run:
        print(f"[train] wandb: {wb_run.url}")

    start = time.time()
    ema_state = None
    ema_start_step = max(0, int(cfg.total_steps * float(getattr(cfg, "tail_ema_start_frac", 0.85))))
    tokens_seen = 0
    last_loss = float("nan")
    for step in range(cfg.total_steps):
        ...
        if getattr(cfg, "tail_ema_decay", 0.0) and step >= ema_start_step:
            sd = model.state_dict()
            if ema_state is None:
                ema_state = {k: v.detach().float().clone() for k, v in sd.items() if torch.is_floating_point(v)}
            else:
                with torch.no_grad():
                    for k, v in sd.items():
                        if k in ema_state and torch.is_floating_point(v):
                            ema_state[k].mul_(cfg.tail_ema_decay).add_(v.detach().float(), alpha=1.0 - cfg.tail_ema_decay)
    ...
    if ema_state is not None:
        save_state = {k: (ema_state[k].to(v.dtype) if k in ema_state else v) for k, v in save_state.items()}
        print("[train] saved tail EMA weights as final checkpoint", flush=True)
    torch.save({"model": save_state, "config": asdict(cfg)}, ckpt_path)
